...
proxyPort = "50000"
# 代理隧道验证信息
proxyUser = "XYWE1650248222130998"
proxyPass = "BRog9Fy1WWKa"
...
def _parsing_dayprice_json(types=None, page=1):
    ...
    while True:
        try:
            text = opener.open(request, timeout=1).read()
            if text == 'null':
                return None
        except Exception:
            continue
        ...
        while True:
            try:
                if src == ct.TICK_SRCS[2]:
                    ...
                    lines = opener.open(re, timeout=10).read()
...
def get_today_ticks(code=None, retry_count=3, pause=0.001):
    ...
    for _ in range(retry_count):
        ...
        try:
            ...
            data_str = opener.open(request, timeout=60).read()
    ...
    for _ in range(retry_count):
        ...
        while True:
            try:
                ...
                #                                              symbol, tdate, pageNo
                #                     ))
                request = Request(ct.TODAY_TICKS_URL % (ct.P_TYPE['http'],
                                                        ct.DOMAINS['vsf'], ct.PAGES['t_ticks'],
                                                        symbol, tdate, pageNo))
                reponse = opener.open(request, timeout=100)
...
def get_today_all():
    ...
    """
    # ct._write_head()
    df_stock_pool.apply_async(_parsing_dayprice_json,args=("hs_a",1),callback=callback_stock_put_queue)


    for i in range(2, ct.PAGE_NUM[1]):
            df_stock_pool.apply_async(_parsing_dayprice_json, args=("hs_a", i), callback=callback_stock_put_queue)
            # newdf = _parsing_dayprice_json('hs_a', i)

            # df = df.append(newdf, ignore_index=True)
    df_stock_pool.apply_async(_parsing_dayprice_json,args=("shfxjs",1),callback=callback_stock_put_queue)
    # df = df.append(_parsing_dayprice_json('shfxjs', 1), # st stock of china A
                   # ignore_index=True)
    # return df


def callback_stock_put_queue(df):
    df_stock_queue.put(df)
